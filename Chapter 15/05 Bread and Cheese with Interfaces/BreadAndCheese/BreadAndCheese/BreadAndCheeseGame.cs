...
namespace BreadAndCheese
{
    ...
    public class TitleSprite : BaseSprite
    {
        ...
        {
            if (game.GetGamePad().Buttons.A == ButtonState.Pressed)
            {
                game.StartGame();
            }
        }
    ...
    public class BatSprite : MovingSprite
    {
        ...
        public override void Update(BreadAndCheeseGame game)
        {
            x = x + (xSpeed * game.GetGamePad().ThumbSticks.Left.X);
            y = y - (ySpeed * game.GetGamePad().ThumbSticks.Left.Y);
            ...
            {
                x = maxDisplayX - spriteRectangle.Width;
            }
            ...
            {
                y = maxDisplayY - spriteRectangle.Height;
            }
    ...
    public class BallSprite : MovingSprite
    {
        ...
        public override void Update(BreadAndCheeseGame game)
        {
            ...
            if (bat.CheckCollision(spriteRectangle))
            {
            ...
            if (targets.CheckCollision(spriteRectangle))
            {
                // ball has hit a target
                targetHitSound.Play();
                game.UpdateScore(20);
                ySpeed = ySpeed * -1;
            }
    ...
    public class DeadlySprite : MovingSprite
    {
        ...
        private SoundEffect deadlyActiveSound;
        private SoundEffectInstance deadlyActiveSoundInstance;
        private SoundEffect deadlyKillSound;

        private void startActiveSound()
        {
            deadlyActiveSoundInstance.Play();
        }

        private void stopActiveSound()
        {
            deadlyActiveSoundInstance.Pause();
        }

        private void playKilledSound()
        {
            stopActiveSound();
            deadlyKillSound.Play();
        }
        ...
        public DeadlySprite(
            ...
            BatSprite inBat,
            BallSprite inBall,
            SoundEffect inDeadlyActiveSound,
            SoundEffect inDeadlyKilledSound)
            : base(
            ...
            inMinDisplayY, inMaxDisplayY, initialX, initialY)
        {
            bat = inBat;
            ball = inBall;
            deadlyActiveSound = inDeadlyActiveSound;
            deadlyKillSound = inDeadlyKilledSound;

            // create active sound instance
            deadlyActiveSoundInstance = deadlyActiveSound.CreateInstance();
            deadlyActiveSoundInstance.IsLooped = true;
        ...
        public override void Update(BreadAndCheeseGame game)
        {
            ...
            if (isDeadly)
            {
                ...
                if (ball.CheckCollision(spriteRectangle))
                {
    ...
    public class KillerSprite : MovingSprite
    {
        ...
        private SoundEffect killerIntro;
        private SoundEffect killerHit;
        ...
        public override void Update(BreadAndCheeseGame game)
        {
            ...
            if (isDeadly)
            {
                if (bat.CheckCollision(spriteRectangle))
                {
                    ...
                    isDeadly = false;
                    // Play low pitched kill on left
                    killerHit.Play(1, -0.5f, 1.0f);
                    // lose some score
                    game.UpdateScore(-10);
                }
                ...
                if (ball.CheckCollision(spriteRectangle))
                {
                    ...
                    isDeadly = false;
                    // Play high pitched kill sound on right
                    killerHit.Play(1, 0.5f, -1.0f);
                    // update the score
                    game.UpdateScore(10);
                }
        ...
        public KillerSprite(
            ...
            BatSprite inBat,
            BallSprite inBall,
            SoundEffect inTangerineIntro,
            SoundEffect inTangerkineKill)
            : base(inSpriteTexture, widthFactor, ticksToCrossScreen,
                inMinDisplayX, inMaxDisplayX,
                inMinDisplayY, inMaxDisplayY,
                inInitialX, inInitialY)
        {
            ...
            killerIntro = inTangerineIntro;
            killerHit = inTangerkineKill;
        }
    }
    ...
    /// </summary>
    public class BreadAndCheeseGame : Microsoft.Xna.Framework.Game, ISpriteBasedGame
    {
        ...
        /// <summary>
        /// Random number generator. Seeded from a constant value
        /// so that the random sequence is always the same one.
        /// </summary>
        ...
        protected override void LoadContent()
        {
            ...
            CheeseBall = new BallSprite(
                ...
                BreadBat,           // bread bat sprite
                TomatoTargets,      // Tomato targets sprite
            ...
            DeadlyPepper = new DeadlySprite(
                ...
                BreadBat,
                CheeseBall,
                Content.Load<SoundEffect>("Sounds/PepperActive"),
                Content.Load<SoundEffect>("Sounds/PepperKill"));
            ...
            for (int i = 0; i < 100; i++)
            {
                ...
                int speed = 100 + rand.Next(600);

                Tangerine = new KillerSprite(
                    Content.Load<Texture2D>("Images/Tangerine"),
                    0.03f,    // a tangerine takes 0.03 of the screen width
                    speed,
                    minDisplayX,
                    maxDisplayX,
                    minDisplayY,
                    ...
                    BreadBat,
                    CheeseBall,
                    Content.Load<SoundEffect>("Sounds/TangerineIntro"),
                    Content.Load<SoundEffect>("Sounds/TangerineHit"));
                GameSprites.Add(Tangerine);
            }

            musicSong = Content.Load<Song>("Sounds/Music");
